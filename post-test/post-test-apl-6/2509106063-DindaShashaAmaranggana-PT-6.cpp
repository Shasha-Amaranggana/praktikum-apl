...
int createEbook(Ebook *katalog, int &jumlahBuku) {
    ...
    if (TahunBuku <= 0) {
        tampilPeringatan(32, "Tahun tidak boleh 0 atau minus!");
        return jumlahBuku;}
    cout << "     Genre   : "; cin >> GenreBuku;
    cout << "     Harga   : "; cin >> HargaBuku;
    if (HargaBuku <= 0) {
        tampilPeringatan(32, "Harga tidak boleh 0 atau minus!");
        return jumlahBuku;}
...
void updateSaldo(Akun *akun, int *indeksLogin) {
    ...
    else if (nominal < 10000 || nominal > 1000000) {
        tampilPeringatan(66, "Nominal tidak boleh kurang dari 10.000 atau lebih dari 1.000.000!");}
    else {
...
/* SUB MENU
════════════════════════════════════════════════════*/
void menuUrutKatalog(Ebook *katalog, int jumlahBuku) {
    string pilihReadKatalog[] = {
        "   1 | Judul",
        "   2 | Penulis",
        "   3 | Genre",
        "   4 | Tahun",
    ...
    while (true) {
        ...
        for (int i = 0; i < 7; i++) {
            if (i == pilih)
                cout << "> " << *(pilihReadKatalog + i) << endl;
            else
                cout << "  " << *(pilihReadKatalog + i) << endl;}
        int hasil = scrollMenu(pilih, 7);
        if (hasil == -1) {

            // A. URUT JUDUL, PENULIS, GENRE, KODE (MERGE SORT)
            if (pilih == 0 || pilih == 1 || pilih == 2 || pilih == 5) {
                system("cls"); judul_subjudul("Urutkan Katalog E-Book"); cout << "" << endl;
                Ebook temp[100];
                for (int i = 0; i < jumlahBuku; i++) {
                    temp[i] = katalog[i];}
                urutJudulPenulisGenreKode_merge(temp, 0, jumlahBuku - 1, pilih + 1);
                readKatalog(temp, jumlahBuku);
                system("pause");}

            // B. URUT TAHUN, HARGA (QUICK SORT)
            else if (pilih == 3 || pilih == 4) {
                system("cls"); judul_subjudul("Urutkan Katalog E-Book"); cout << "" << endl;
                Ebook temp[100];
                for (int i = 0; i < jumlahBuku; i++) {
                    temp[i] = katalog[i];}
                urutTahunHarga_quick(temp, 0, jumlahBuku - 1, pilih - 2);
                readKatalog(temp, jumlahBuku);
                system("pause");}
...
void menuCariKatalog(Ebook *katalog, int jumlahBuku) {
    string pilihReadKatalog[] = {
        "   1 | Judul",
        "   2 | Penulis",
        "   3 | Genre",
    ...
    while (true) {
        system("cls"); judul_subjudul("Cari Katalog E-Book"); cout << endl;
        readKatalog(katalog, jumlahBuku);
        ...
        if (hasil == -1) {
            ...
            // A. CARI JUDUL, PENULIS, GENRE (LINEAR SEARCH)
            if (pilih == 0 || pilih == 1 || pilih == 2) {
                system("cls"); judul_subjudul("Cari Katalog E-Book"); cout << "" << endl;
                string keyword; cout << "Masukkan keyword: "; cin >> keyword;
                Ebook hasil[100];
                int jumlahHasil = cariJudulPenulisGenre_linear(katalog, jumlahBuku, hasil, keyword);
                if (jumlahHasil > 0) {
                    cout << endl; readKatalog(hasil, jumlahHasil);}
                else {
                    tampilPesan(22, "Data tidak ditemukan!");}
                system("pause");}

            // B. CARI TAHUN, HARGA (LINEAR SEARCH)
            else if (pilih == 3 || pilih == 4) {
                system("cls"); judul_subjudul("Cari Katalog E-Book"); cout << "" << endl;
                int min, max;
                cout << "Masukkan nilai minimum: "; cin >> min;
                cout << "Masukkan nilai maksimum: "; cin >> max;
                Ebook hasil[100];
                int jumlahHasil = cariTahunHargaRange_linear(katalog, jumlahBuku, hasil, min, max, pilih - 2);
                if (jumlahHasil > 0) {
                    cout << endl; readKatalog(hasil, jumlahHasil);}
                else {
                    tampilPesan(22, "Data tidak ditemukan!");}
            ...
            // C. CARI KODE (BINARY SEARCH)
                ...
                for (int i = 0; i < jumlahBuku; i++) {
                    temp[i] = katalog[i];}
                urutJudulPenulisGenreKode_merge(temp, 0, jumlahBuku - 1, 4);
                int index = cariKode_binary(temp, jumlahBuku, keyword);
                if (index != -1) {
                    cout << endl; readKatalog(&temp[index], 1);}
                else {
                    tampilPesan(22, "Data tidak ditemukan!");}
                system("pause");}
        ...
        if (hasil == -1) {
            ...
            if (pilih == 0) {
                ...
                while (true) {
                    ...
                    if (hasil == -1) {
                        if (pilih == 0) {
                            menuUrutKatalog(katalog, jumlahBuku);}
                        else if (pilih == 1) {
                            menuCariKatalog(katalog, jumlahBuku);}
                        else {
                            break;}}}}
