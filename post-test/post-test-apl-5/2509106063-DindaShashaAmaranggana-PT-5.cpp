...
void merge(Ebook *katalog, int kiri, int tengah, int kanan, int jenis) {
    ...
    while (i < n1 && j < n2) {
        bool kondisi = false;
        if (jenis == 1){
            kondisi = (L + i)->judul <= (R + j)->judul;}
        else if (jenis == 2){
            kondisi = (L + i)->penulis <= (R + j)->penulis;}
        else{
            kondisi = L[i].genre <= R[j].genre;}
        if (kondisi) {
            *(katalog + k) = *(L + i);
            i++;}
        else {
...
void mergeSort(Ebook *katalog, int kiri, int kanan, int jenis) {
    if (kiri < kanan) {
        int tengah = kiri + (kanan - kiri) / 2;
        mergeSort(katalog, kiri, tengah, jenis);
        mergeSort(katalog, tengah + 1, kanan, jenis);
...
int partition(Ebook *katalog, int low, int high, int jenis) {
    ...
    for (int j = low; j < high; j++) {
        bool kondisi = false;
        if (jenis == 1) {
            kondisi = (katalog + j)->harga > pivot.harga;} 
        else if (jenis == 2) {
            kondisi = (katalog + j)->tahun > pivot.tahun;}
        if (kondisi) {
            i++;
...
void quickSort(Ebook *katalog, int low, int high, int jenis) {
    if (low < high) {
        int pi = partition(katalog, low, high, jenis);
        quickSort(katalog, low, pi - 1, jenis);
        quickSort(katalog, pi + 1, high, jenis);}
}

void bubbleSort(Ebook *katalog, int jumlahBuku) {
    for (int i = 0; i < jumlahBuku - 1; i++) {
        for (int j = 0; j < jumlahBuku - i - 1; j++) {
            if ((katalog + j)->id > (katalog + j + 1)->id) {
                ...
                while (true) {
                    ...
                    if (hasil == -1) {
                        ...
                        if (pilih == 0 || pilih == 1 || pilih == 2) {
                            ...
                            for (int i = 0; i < jumlahBuku; i++) {
                                temp[i] = katalog[i];}
                            mergeSort(temp, 0, jumlahBuku - 1, pilih + 1);
                            ...
                            for (int i = 0; i < jumlahBuku; i++) {
                                temp[i] = katalog[i];}
                            if (pilih == 3) {
                                quickSort(temp, 0, jumlahBuku - 1, 2);}
                            else if (pilih == 4) {
                                quickSort(temp, 0, jumlahBuku - 1, 1);}
                            readKatalog(temp, jumlahBuku);
                            system("pause");}
...
int main() {
    ...
    Ebook katalog[100] = {
        ...
        {22, "The Walled City", "Ryan Graudin", 2014, "Drama", 50000},
        {31, "City of Lost Souls", "Cassandra Clare", 2012, "Fantasi", 60000},
        {9, "The Trials of Apollo 2", "Rick Riordan", 2017, "Fantasi", 50000},
        {19, "The Trials of Apollo 3", "Rick Riordan", 2018, "Fantasi", 50000}};
